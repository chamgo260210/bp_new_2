...
public class ConceptJourneyService {
        ...
        try {
            ...
            for(int round=0;round<=batch.getMaxReplacementRounds()&&accepted.size()<batch.getTargetEligibleCount()&&sequence<batch.getMaxInspectedCandidates();round++){
                ...
                for(JsonNode candidate:generated.get("concepts")){
                    ...
                    String legalInput=conceptLegalInput(context,candidate);
                    TaskRun legalTask=createTask(ownerId,context.project(),TaskType.CONCEPT_LEGAL_VALIDATION,"CONCEPT_DRAFT",draft.getId().toString(),legalInput);
                    JsonNode legal=execute(legalTask,this::validateConceptLegal);
                    boolean pass="PASS".equals(legal.path("status").asText());
                    ((ObjectNode)candidate).set("legalTrace",legal.path("legalTrace"));
                    draft.legal(pass?ConceptDraft.LegalStatus.PASS:ConceptDraft.LegalStatus.FAIL_LEGAL,legal.path("reasons").toString(),legal.path("violatedStructureKeys").toString(),pass?null:String.join(" ",strings(legal.path("reasons"))),candidate.toString());conceptDrafts.save(draft);
                    batch.inspected(pass); if(pass)accepted.add(draft);else{negatives.addAll(strings(legal.path("reasons")));negatives.addAll(strings(legal.path("violatedStructureKeys")));}
                }
            }
    ...
    private String conceptGenerationInput(Context c,int desired,int round,List<String> negatives,List<ConceptDraft> accepted){ObjectNode value=mapper.createObjectNode();value.put("desiredCount",desired);value.put("round",round);value.set("ideaOrigin",parse(c.origin().getSnapshotJson()));value.set("lockedValues",parse(c.origin().getConfirmedValuesJson()));value.set("legalGuardrail",guardrailJson(c.guardrail()));value.set("negativeConstraints",mapper.valueToTree(new LinkedHashSet<>(negatives)));ArrayNode prior=value.putArray("acceptedConcepts");accepted.forEach(d->prior.add(parse(d.getDraftJson())));return taskInput("concept-eligibility-generation",value.toString());}
    private String conceptLegalInput(Context c,JsonNode candidate){ObjectNode value=mapper.createObjectNode();value.set("conceptDraft",candidate);value.set("guardrails",guardrailJson(c.guardrail()));value.set("newBusinessActivities",candidate.path("newBusinessActivities"));ObjectNode input=(ObjectNode)parse(taskInput("concept-legal-validation",value.toString()));input.put("validationMode","GUARDRAIL");input.put("guardrailVersionId",c.guardrail().getId());return input.toString();}
    ...
    private void validateConceptLegal(JsonNode r){if(r==null||!Set.of("PASS","FAIL_LEGAL").contains(r.path("status").asText()))throw invalid();for(String f:List.of("reasons","violatedStructureKeys","legalTrace"))array(r,f);}
