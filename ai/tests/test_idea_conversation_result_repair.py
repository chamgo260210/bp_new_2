...
def test_result_model_rejects_noncanonical_provider_types(mutate):
    ...
    with pytest.raises(ValidationError):
        OpportunityBriefDraftResult.model_validate(candidate)
...
def test_initial_invalid_result_is_repaired_exactly_once(monkeypatch, caplog):
    ...
    assert calls[0]["response_schema"] == OpportunityBriefDraftResult.model_json_schema()
    assert calls[1]["response_schema"] == OpportunityBriefDraftResult.model_json_schema()
...
def test_provider_request_uses_pydantic_json_schema(monkeypatch):
    ...
    schema = OpportunityBriefDraftResult.model_json_schema()
