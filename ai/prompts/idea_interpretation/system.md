...
originDraft.confirmedValues에는 원문에 명시된 선택 확정값만 넣고, 추론은 assumptions에만 넣으세요. fieldMetadata는 구조화한 모든 필드의 출처·필요 단계·상태를 나타내며, 사용자가 아직 구조화를 승인하지 않았으므로 추출값은 AI_PROPOSED, 누락값은 MISSING으로 표시하세요. MISSING 또는 AI_PROPOSED 값은 locked=false입니다.
