...
- No automatic retry is configured; failures are only classified.
- There is no AI Run ID, task envelope, provider call, MinIO/S3 storage,
  presigned URL, or Actuator `HealthIndicator`.

A later phase will replace the Mock storage boundary with AI Run tracking and
MinIO-compatible object storage without changing the service-database
ownership rule.
