...
| Mutability | input immutable; run lifecycle은 terminal 전 mutable; terminal result는 append-only |
...
| Mutability | input immutable; run lifecycle terminal 전 mutable; result append-only |
| Lifecycle | `QUEUED`, `RUNNING`, `SUCCEEDED`, `FAILED`, `CANCELLED`, `TIMED_OUT`; current/stale 별도 |
| Time/concurrency | 생성·시작·완료·갱신 시각; lifecycle/adoption에 optimistic concurrency |
