...
2. Spring이 Project owner, Service Policy, type, size, filename과 content를 검증한다.
...
6. parser는 IdeaSource 처리 구성요소이며 DOCX 전용 Workflow를 만들지 않는다.
...
AI Server는 Storage에 접근하지 않는다. binary 전달은 Spring-mediated 방식이어야 하며 선택지는 [OD-002](../product/OPEN_DECISIONS.md)에 남는다. Spring은 content type, magic bytes, size, checksum, expected task/result identity를 검증한 후 Storage에 저장한다.

## Lifecycle

RDB reference가 artifact lifecycle의 source of truth다. soft delete, retention, quarantine, orphan reconciliation과 report export 보존 기간의 상세는 구현 Phase에서 결정한다. legacy artifact는 참조 해제 후 삭제한다.
