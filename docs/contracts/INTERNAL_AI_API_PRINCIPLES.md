...
P2.5는 `IDEA_INTERPRETATION` Task type을 지원한다. 이 task에는 exact extracted text/chunks, source-safe labels, readiness/normalization options, contract version, correlation/execution identifier, deadline과 canonical input hash만 포함한다. AI Server가 resolve할 RDB identifier, Storage URL/object key/presigned URL/local path, FILE bytes, JWT, user credential와 Spring entity serialization은 포함하지 않는다.
