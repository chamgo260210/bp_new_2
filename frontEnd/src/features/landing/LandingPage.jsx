...
function FeatureSection() { return <section id="features" className="landing-section landing-features" aria-labelledby="features-title"><div className="landing-container"><p className="landing-eyebrow">CORE FEATURES</p><h2 id="features-title">검증에 필요한 정보를,<br />한 프로젝트 안에서 관리하세요.</h2><div className="feature-grid">{featureItems.map(([title, description, size], index) => <article className={`feature-card ${size}`} key={title}><span className="feature-card__number">0{index + 1}</span><h3>{title}</h3><p>{description}</p>{index === 0 && <div className="feature-visual feature-visual--document"><span>Before<br /><b>25페이지 사업계획서</b></span><i aria-hidden="true">→</i><span>After<br /><b>핵심 검토 항목</b></span></div>}{index === 1 && <div className="feature-visual"><b>근거 있음</b><b>추가 확인 필요</b><b>판단 보류</b></div>}</article>)}</div></div></section>; }
